...
async function uploadToVersion(
  expenseId: number,
  versionId: number,
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!inserted) { res.status(404).json({ error: "Expense version not found" }); return; }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!inserted) { res.status(404).json({ error: "Version not found" }); return; }
