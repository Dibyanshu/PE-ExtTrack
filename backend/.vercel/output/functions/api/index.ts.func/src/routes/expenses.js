...
import { db, pool, expenses, expenseVersions, particularsMaster, uomMaster, paymentStatusMaster, projectMaster, vendorMaster, documents, } from "@workspace/db";
...
router.get("/expenses", requireAuth, async (req, res) => {
    const { from, to, projectId, particularId, paymentStatusId, vendorId, voucherType, voucherNumber, page = "1", limit = "20", } = req.query;
    ...
    if (projectId)
        conditions.push(eq(expenses.projectId, Number(projectId)));
    if (particularId)
        conditions.push(eq(expenseVersions.particularId, Number(particularId)));
    if (paymentStatusId)
        conditions.push(eq(expenseVersions.paymentStatusId, Number(paymentStatusId)));
    if (vendorId)
        conditions.push(eq(expenses.vendorId, Number(vendorId)));
    if (voucherType === "payment" || voucherType === "receive") {
        conditions.push(eq(expenses.voucherType, voucherType));
    }
    if (voucherNumber)
        conditions.push(like(expenseVersions.voucherNumber, `%${voucherNumber}%`));
...
router.put("/expenses/:id", requireRole("expense_entry"), async (req, res) => {
    ...
    const [prevVersion] = await db
        ...
        .limit(1);
    const { expenseDate, particularId, description, transactionDetails, uomId, quantity, pricePerUnit, invoiceNumber, paymentStatusId, } = req.body;
    ...
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        await conn.execute("UPDATE expenses SET current_version_id = ? WHERE id = ?", [newVersionId, expenseId]);
        await conn.commit();
    }
    catch (err) {
        await conn.rollback();
