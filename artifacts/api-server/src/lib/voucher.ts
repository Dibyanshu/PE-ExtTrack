...
export async function nextVoucherNumber(type: VoucherType): Promise<string> {
  ...
  try {
    ...
    await conn.execute(
      "SELECT id FROM voucher_sequence WHERE id = ? FOR UPDATE",
      [seqId],
    );
    // Increment
    await conn.execute(
      "UPDATE voucher_sequence SET current_value = current_value + 1 WHERE id = ?",
      [seqId],
    );
    // Read back the new value on the same connection (no LAST_INSERT_ID needed)
    const [rows] = await conn.execute(
      "SELECT current_value AS val, prefix FROM voucher_sequence WHERE id = ?",
      [seqId],
    ) as [Array<{ val: number; prefix: string }>, unknown];
    await conn.commit();
